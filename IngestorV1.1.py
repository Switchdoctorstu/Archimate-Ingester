...
- Bulk import of names from text lists
...
class ArchiIngestorApp:
    def __init__(self, root):
        ...
        self.root.geometry("1100x800")  # Increased height for new section
    ...
    def build_gui(self):
        ...
        bulk_frame = tk.LabelFrame(right_frame, text="Bulk Import (paste list of names)")
        bulk_frame.pack(fill="x", padx=4, pady=4)

        tk.Label(bulk_frame, text="Paste list of names (one per line):").pack(anchor="w")
        self.bulk_text = tk.Text(bulk_frame, height=6)
        self.bulk_text.pack(fill="x", expand=True)

        bulk_buttons = tk.Frame(bulk_frame)
        bulk_buttons.pack(fill="x")
        tk.Button(bulk_buttons, text="Clear Bulk Import", command=self.clear_bulk).pack(side="left", padx=4, pady=4)
        tk.Button(bulk_buttons, text="Convert to Paste Format", command=self.convert_bulk_to_paste).pack(side="left", padx=4, pady=4)
    ...
    def save_as(self):
        ...
        try:
            ...
            # Manual XML generation to avoid namespace issues
            xml_lines = []
            xml_lines.append('<?xml version="1.0" encoding="UTF-8"?>')
            model_name = escape_xml(self.model.get("name", ""))
            model_id = escape_xml(self.model.get("id", ""))
            model_version = escape_xml(self.model.get("version", ""))
            xml_lines.append(f'<archimate:model xmlns:xsi="{XSI}" xmlns:archimate="{ARCHIMATE}" name="{model_name}" id="{model_id}" version="{model_version}">')
            
            # Process all folders
            for folder in self.model.findall(".//folder"):
                folder_name = escape_xml(folder.get("name", ""))
                folder_id = escape_xml(folder.get("id", ""))
                folder_type = escape_xml(folder.get("type", ""))
                folder_attrs = f'name="{folder_name}" id="{folder_id}" type="{folder_type}"'
                xml_lines.append(f'  <folder {folder_attrs}>')
                
                # Process elements in this folder
                for element in folder.findall(".//element"):
                    element_type = escape_xml(element.get(f"{{{XSI}}}type", ""))
                    element_name = escape_xml(element.get("name", ""))
                    element_id = escape_xml(element.get("id", ""))
                    element_attrs = f'xsi:type="{element_type}" name="{element_name}" id="{element_id}"'
                    
                    # Check for child elements (like in diagrams)
                    children = list(element)
                    if children:
                        xml_lines.append(f'    <element {element_attrs}>')
                        for child in children:
                            child_tag = child.tag
                            # Handle xsi:type attribute properly
                            child_attribs = {}
                            for k, v in child.attrib.items():
                                if k == f"{{{XSI}}}type":
                                    child_attribs["xsi:type"] = escape_xml(v)
                                else:
                                    child_attribs[k] = escape_xml(v)
                            
                            child_attrs_str = " ".join([f'{k}="{v}"' for k, v in child_attribs.items()])
                            
                            if list(child):  # If child has its own children
                                xml_lines.append(f'      <{child_tag} {child_attrs_str}>')
                                for grandchild in child:
                                    grandchild_tag = grandchild.tag
                                    # Handle xsi:type attribute for grandchildren too
                                    grandchild_attribs = {}
                                    for k, v in grandchild.attrib.items():
                                        if k == f"{{{XSI}}}type":
                                            grandchild_attribs["xsi:type"] = escape_xml(v)
                                        else:
                                            grandchild_attribs[k] = escape_xml(v)
                                    
                                    grandchild_attrs_str = " ".join([f'{k}="{v}"' for k, v in grandchild_attribs.items()])
                                    xml_lines.append(f'        <{grandchild_tag} {grandchild_attrs_str}/>')
                                xml_lines.append(f'      </{child_tag}>')
                            else:
                                xml_lines.append(f'      <{child_tag} {child_attrs_str}/>')
                        ...
                        if documentation is not None and documentation.text:
                            doc_text = escape_xml(documentation.text)
                            xml_lines.append(f'    <element {element_attrs}>')
                            xml_lines.append(f'      <documentation>{doc_text}</documentation>')
                            xml_lines.append('    </element>')
                        else:
                            xml_lines.append(f'    <element {element_attrs}/>')
                
                # Process relationships in this folder
                for relationship in folder.findall(".//relationship"):
                    rel_type = escape_xml(relationship.get(f"{{{XSI}}}type", ""))
                    rel_id = escape_xml(relationship.get("id", ""))
                    rel_source = escape_xml(relationship.get("source", ""))
                    rel_target = escape_xml(relationship.get("target", ""))
                    rel_attrs = f'xsi:type="{rel_type}" id="{rel_id}" source="{rel_source}" target="{rel_target}"'
                    xml_lines.append(f'    <relationship {rel_attrs}/>')
                
                xml_lines.append('  </folder>')
            
            # Close the model tag
            xml_lines.append('</archimate:model>')
    ...
    def refresh_tree(self):
        ...
        for folder in folders:
            ...
            # Add relationships in this folder
            for relationship in folder.findall(".//relationship"):
                rel_type = relationship.get(f"{{{XSI}}}type", "").split(":")[-1] if ":" in relationship.get(f"{{{XSI}}}type", "") else relationship.get(f"{{{XSI}}}type", "")
                rel_source = relationship.get("source", "")
                rel_target = relationship.get("target", "")
                
                rel_label = f"{rel_type}: {rel_source} -> {rel_target}"
                self.treeview.insert(folder_id, "end", text=rel_label)
    ...
    def clear_bulk(self):
        """Clear the bulk import text area."""
        self.bulk_text.delete("1.0", "end")

    def convert_bulk_to_paste(self):
        """Convert bulk text (list of names) to paste area format."""
        bulk_text = self.bulk_text.get("1.0", "end").strip()
        if not bulk_text:
            messagebox.showinfo("No content", "Bulk Import area is empty.")
            return
        
        # Get the currently selected type from the dropdown
        selected_type = self.type_var.get().strip()
        if not selected_type:
            messagebox.showwarning("No type selected", "Please select an element type from the dropdown first.")
            return
        ...
        for line in lines:
            ...
            # 1. Just a name: "Server1" -> "BusinessActor | Server1"
            # 2. Name with description separated by colon/dash: "Server1: Main database server" -> "BusinessActor | Server1 | description=Main database server"
            # 3. Name with other metadata
            ...
            if ":" in line:
                ...
                paste_lines.append(f"{selected_type} | {name} | description={description}")
                ...
                # Just a name
                paste_lines.append(f"{selected_type} | {line}")
        
        # Append to existing paste area or replace if empty
        current_paste = self.paste_text.get("1.0", "end").strip()
        if current_paste:
            # Append with newlines
            new_content = current_paste + "\n" + "\n".join(paste_lines)
        else:
            # Replace
            new_content = "\n".join(paste_lines)
        
        self.paste_text.delete("1.0", "end")
        self.paste_text.insert("1.0", new_content)
        
        # Update the preview
        self.update_staged_preview()
        
        # Clear bulk area after successful conversion
        self.clear_bulk()
        
        messagebox.showinfo("Converted", f"Converted {len(lines)} items to paste format.")
    ...
    def insert_from_paste(self):
        ...
        for ln in lines:
            ...
            # Create element WITHOUT namespace prefix
            new_el = ET.Element("element", attribs)
